...
var AddFeedBack =React.createClass({
                    ...
                    <ScrollView>
                        <View style={styles.addFeedback}>
                            <View style={styles.backTop}>
                                <View style={styles.backTopIn}>
                                    <View style={styles.backTopT}>
                                        ...
                                        <Text style={styles.imgTitleT}>对此次订购进行评价</Text>
                                    ...
                                    <View style={styles.pingZ}>
                                        <View style={styles.pingJ}>
                                            <View style={[styles.uniforms,styles.rightMargin]}><Text style={styles.fontStyleB}>校服款式</Text></View>
                                            {this._initStarView('ks',this.state.ksStarCount)}
                                        </View>
                                        <View style={styles.pingJ}>
                                            <View style={[styles.uniforms,styles.rightMargin]}><Text style={styles.fontStyleB}>校服价格</Text></View>
                                            {this._initStarView('jg',this.state.jgStarCount)}
                                        </View>
                                        <View style={styles.pingJ}>
                                            <View style={[styles.uniforms,styles.rightMargin]}><Text style={styles.fontStyleB}>生产厂商</Text></View>
                                            {this._initStarView('cs',this.state.csStarCount)}
                                        </View>
                                        <View style={styles.pingJ}>
                                            <View style={[styles.uniforms,styles.rightMargin]}><Text style={styles.fontStyleB}>购买体验</Text></View>
                                    ...
                                    <View style={styles.backBottomB}>
                                        ...
                                        <Text style={styles.imgTitleT}>反馈留言</Text>
                                        <Image source={require('../../img/feedback/line.png')} resizeMode='contain' style={[styles.imgRightB,{width:Util.size.width/2-80}]} />
                                    </View>
                                    <View style={styles.backCenter}>
                                        <TextInput onChangeText={this.msgChangeFunc.bind(this)} autoCapitalize="none" autoCorrect={false} style={styles.backCon} placeholderTextColor="#b0b0b0" placeholder="请给我们一些建议与留言，200字以内"></TextInput>
                                    </View>
                                </View>
                            </View>
                            <TouchableOpacity onPress={this.FuncFetchData.bind(this,{
                                ...
                                <View style={styles.sureBtn}><Text style={styles.surebtn}>确定</Text></View>
                            </TouchableOpacity>
                        </View>
                    </ScrollView>
                }
            ...
            if(that.state.isDisable){
                 if(that.state.ksStarCount!=0 && that.state.jgStarCount!=0 && that.state.csStarCount!=0 && that.state.tyStarCount!=0){
                    that.setState({isDisable:false});  //按钮设置为不可点击，避免重复提交
                     ...
                     reject('noNet1');
                     ToastAndroid.show('请完成反馈评价后提交。', 2000);
                 }
            }
        });
    },
    ...
    _initStarView:function(optionType,starCount){
        ...
        itemList[j]=(<View style={[styles.uniforms,styles.weiZ]}><Text style={styles.fontStyleS}>{obj[starCount]}</Text></View>);
        return itemList;
    },
     //监测网络状态
