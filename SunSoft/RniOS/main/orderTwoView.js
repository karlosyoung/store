...
var OrderTwoView =React.createClass({
                    ...
                    <View style={styles.orderList}>
                        <View style={styles.orderSum}>
                            ...
                            <View style={styles.shoAddress}>
                                <View style={{width:Util.size.width-20, height:85,justifyContent:'center',alignItems:'center'}}>
                                    <Image source={require('../../img/order/border.png')} style={{width: Util.size.width-20,height:85} } />
                                    <View style={styles.showIn}>
                                        <View style={styles.naAdd}>
                                            ...
                                            <View style={styles.nameMAx}><Text allowFontScaling={false} style={styles.artailColor} numberOfLines={1} >{this.state.orderOneData.consignee}</Text></View>
                                            <Image source={require('../../img/order/iphone.png')} style={{width: 12, height: 19,marginRight:10,marginTop:3} } />
                                            <View style={styles.phoneMAx}><Text allowFontScaling={false} style={styles.artailColor} numberOfLines={1}>{this.state.orderOneData.tel}</Text></View>
                                        </View>
                            ...
                            <View style={styles.shoHuo}>
                                <View style={styles.yiHang}><Text allowFontScaling={false} style={styles.peiPay}>支付配送：</Text><Text allowFontScaling={false} style={styles.peiLine}>在线支付 集中配送至学校</Text></View>
                                <View style={styles.yiHang}><Text allowFontScaling={false} style={styles.peiPay}>商品数量：</Text><Text allowFontScaling={false} style={styles.sunGood}>共{this.state.data.goodsNumber}件</Text></View>
                                <View style={styles.yiHang}><Text allowFontScaling={false} style={styles.peiPay}>商品总额：</Text><Text allowFontScaling={false} style={[styles.red,{ height:25,lineHeight:20}]}>{Util.FuncRmbFormat(this.state.data.price)}</Text></View>
                            </View>
                            {
                                status=='02'|| this.state.btnDisable==false?
                                    <TouchableOpacity  onPress={this._popOrder.bind(this)}>
                                        <View style={styles.nextBu}><Text allowFontScaling={false} style={styles.next}>确认提交</Text></View>
                                    </TouchableOpacity>
                                :
                                    <View style={styles.xiaNext}><Text allowFontScaling={false} style={styles.next}>确认提交</Text></View>
                            }
                        </View>
    ...
    _onRefresh: function () {
      ...
      //选择支付方式弹出框回调函数
        _callback() {
         ...
         this.props.navigator.push({
            ...
            params:{
                initData:{
                                   isNative:false,   //true表示原生跳转 false 表示RN跳转
                    token:this.state.token,
     ...
     //提交订单跳转到弹出框
        _popOrder:function(){
            ...
            RNBridgeModule.toast('您选择的商品中有已下架的商品！');
            }
        },
    ...
    _retailCreateOrder:function(){
        ...
        return new Promise(function (resolve, reject) {
             var _type='02';//订单的来源  01-PC端  02-IOS端 03-android
                    ...
                    <View style={styles.listDetail}>
                        {
                            item.status=='02'?
                                <View style={[styles.marginL,{width:90, marginLeft:20}]}>
                                    <Image source={{uri: item.goodsImg+'!m166x166.jpg'}} style={{width: 90, height: 90}} />
                        ...
                        <View style={[styles.guiGe,styles.marginL]}>
                            ...
                            <Text allowFontScaling={false} style={styles.marginBet}>尺码：{item.goodsAttrValue}</Text>
                            <Text allowFontScaling={false} style={styles.marginBet}>数量：X{item.goodsNumber}</Text>
                            <Text allowFontScaling={false} style={styles.marginBet}>价格：<Text allowFontScaling={false} style={styles.red}>{Util.FuncRmbFormat(item.shopPrice)}</Text></Text>
                        </View>
                    </View>
                );
            }
